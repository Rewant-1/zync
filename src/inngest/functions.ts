  ...
  async ({ event, step }) => {
    ...
    const [sandboxId, previousMessages] = await Promise.all([
      step.run("get-sandbox-id", async () => {
        ...
        await sandbox.setTimeout(60_000 * 15);
    ...
    console.log(`🎯 Starting model attempts. Total models to try: ${modelsToTry.length}`);
    console.log(`📋 Models list: ${JSON.stringify(modelsToTry)}`);

    for (const model of modelsToTry) {
      attemptCount++;
      console.log(`🚀 Attempt ${attemptCount}/${modelsToTry.length} - Starting with model: ${model}`);
      
      try {
        console.log(`🤖 Creating agent for model: ${model}`);
              ...
              handler: async (
                ...
                const res = await step?.run("createFiles", async () => {
                  try {
                    ...
                    // Write files sequentially to ensure proper dependency order
        ...
        console.log(`🔗 Creating network for model: ${model}`);
        ...
        console.log(`▶️ Running network for model: ${model}`);
        ...
        console.log(`❌ Model ${model} (attempt ${attemptCount}/${modelsToTry.length}) failed:`, error);
        
        // Always continue to next model unless it's the last one
        // This ensures we try ALL models before giving up
        if (attemptCount < modelsToTry.length) {
          console.log(`🔄 Continuing to next model... (${modelsToTry.length - attemptCount} models remaining)`);
          continue;
        } else {
          console.log(`🔚 This was the last model, no more fallbacks available`);
        }
      }
    }
    ...
    // Run title and response generation in parallel for faster execution
    const [
